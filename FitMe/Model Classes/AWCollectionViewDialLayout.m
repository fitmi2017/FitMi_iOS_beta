...
-(id)initWithRadius: (CGFloat) radius andAngularSpacing: (CGFloat) spacing andCellSize: (CGSize) cell andAlignment:(WheelAlignmentType)alignment andItemHeight:(CGFloat)height andXOffset: (CGFloat) xOff currentYPosition:(CGFloat)ypos{
    if ((self = [super init]) != NULL)
    {
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    int maxVisibleOnScreen = 360 ;/// self.AngularSpacing + 2;
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"layoutAttributesForItemAtIndexPath===%ld",(long)indexPath.item);
    ...
    if( self.wheelType == WHEELALIGNMENTLEFT){
       // scaleFactor = fmax(0.6, 1 - fabs( newIndex *0.25));
        deltaX =0.0f; //self.cellSize.width/2;
        theAttributes.center = CGPointMake(-self.dialRadius + self.xOffset  , self.YPosition + self.collectionView.contentOffset.y);

...
-(BOOL)deleteFavMealData:(NSString *)foodId withUserId:(NSString *)userId
{
    ...
    if (![DbController mDBHandler].inUse) {
        ...
        NSString *sqlQry=[NSString stringWithFormat:@"delete from fitmi_fav_meal_log  where  id='%@'",foodId];
...
-(NSMutableArray*)getUniqueFavDate:(NSString *)userProfileID withMealID:(NSString*)selectedMealId
{
    ...
    if (![DbController mDBHandler].inUse) {
        ...
        {
            sqlQry = [NSString stringWithFormat:@"SELECT log_time  FROM fitmi_fav_meal_log where user_profile_id='%@'  GROUP BY DATE(log_time) ORDER BY log_time DESC",userProfileID];
        }
        else
        {
            sqlQry = [NSString stringWithFormat:@"SELECT log_time  FROM fitmi_fav_meal_log where meal_id='%@' AND user_profile_id='%@'  GROUP BY DATE(log_time) ORDER BY log_time DESC",selectedMealId,userProfileID];
        }
...
-(NSMutableArray*)getFavMealFoodLog:(NSString*)selectedId withSelectedDate:(NSString *)selectedDate withUserId:(NSString*)selectedUserId{
    ...
    if (![DbController mDBHandler].inUse) {
        ...
        if([selectedId isEqualToString: @"" ]){
            sqlQry = [NSString stringWithFormat:@"select * from fitmi_fav_meal_log where  user_profile_id='%@' AND log_time between '%@ 00:00:01' AND '%@ 23:59:59'  group by reference_food_id",selectedUserId,selectedDate,selectedDate];
        }
        else{
            sqlQry = [NSString stringWithFormat:@"select * from fitmi_fav_meal_log where meal_id='%@' AND user_profile_id='%@' AND log_time between '%@ 00:00:01' AND '%@ 23:59:59'  group by reference_food_id",selectedId,selectedUserId,selectedDate,selectedDate];
        }
        ResultSet *rs=[[DbController mDBHandler] executeQuery:sqlQry];
        while ([rs next]) {
